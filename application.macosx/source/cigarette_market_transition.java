...
public class cigarette_market_transition extends PApplet {
...
public void controlEvent(ControlEvent theEvent) {
  ...
  if(theEvent.isFrom(d1)){
    xaxis = PApplet.parseInt(theEvent.getGroup().getValue());
  }
  if(theEvent.isFrom(d2)){
    yaxis = PApplet.parseInt(theEvent.getGroup().getValue());
  }

  if(theEvent.isFrom(checkbox)){
    println(checkbox.getArrayValue());
    for (i=0; i<13; i++) {
      brandcheck[i] = PApplet.parseInt(checkbox.getArrayValue()[i]);
    }
    checksum = 0;
    for(j=0; j<13; j++){
...
public void draw() {
  ...
  switch(xaxis){
    ...
    case 3: //price
      for (i = 500; i > 0; i -= 100){
        stroke(255);
        j = PApplet.parseInt(i*1.3f-90); line(j, 330, j, 315); text("\u00a5"+i, j, 340);
        stroke(150);
        j = PApplet.parseInt(i*1.3f-90); line(j, 315, j, 30);
      }
    default:
      break;
  }
  ...
  switch(yaxis){
    ...
    case 3: //price
      for (i = 500; i > 100; i -= 100){
        stroke(255);
        j = PApplet.parseInt(450-(i*0.8f)); line(40, j, 55, j); text("\u00a5"+i, 20, j);
        stroke(150);
        j = PApplet.parseInt(450-(i*0.8f)); line(55, j, 585, j);
      }
      break;
    default:
      break;
  ...
  for (int j = 0; j < 13; j++) {
    ...
    Coloring(colorbrand[j]);
      ...
      fill(red, green, blue, PApplet.parseInt((1-checksum)*128+checksum*(2.2f*(100-opacity))));
      drawEllipse(hairetu[j], brandcheck[j], brandname[j]);
    }
  }
...
public int xaxis(int a, int b) {
  switch(b){
    ...
    case 2: //tar
      return a*23+60;
    case 3: //price
      return PApplet.parseInt(a*1.3f-90);
    default:
      return 9999;
  }
}

public int yaxis(int a, int b) {
  switch(b){
    case 1: //share
      return 300-(a*2);
    case 2: //tar
      return 315-(a*13);
    case 3: //price
      return PApplet.parseInt(450-a*0.8f);
    default:
      return 9999;
  }
}

public void drawEllipse(int[][] brand, int check, String test){
  if (check == 0){ //no checks
    r = sqrt(brand[year-1998][0]);
    x = xaxis(brand[year-1998][xaxis], xaxis);
    y = yaxis(brand[year-1998][yaxis], yaxis);
    int op = PApplet.parseInt((1-checksum)*255+checksum*(2.2f*(100-opacity)));
    if(x>40 && y<330){
      stroke(255, op);
      ellipse(x, y, r/2, r/2);
      fill(0, op);
...
public void drawLine(int[][] brand, int red, int green, int blue){
...
public void Coloring(int colorbrand){
  switch(colorbrand){
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "cigarette_market_transition" };
    if (passedArgs != null) {
